...
**Date:** 2026-03-27 (updated from 2026-02-21)  
**Version:** 2.0  
...
8. [Refactoring Roadmap (original)](#section-8-refactoring-roadmap)  
9. [Progress Since v1.0 — What Changed](#section-9-progress-since-v10)  
10. [Gradual Clean Architecture Plan](#section-10-gradual-clean-architecture-plan)  
...
> **NOTE (v2.0):** Several items from the original handover have been resolved. See Section 9 for full progress tracking.

| Issue | Severity | Description | Status (March 2026) |
|-------|----------|-------------|---------------------|
| **God File: views.py (10,911 lines)** | CRITICAL | Every view in a single file. | ✅ RESOLVED — split into 13 modules under `views/` |
| **God File: models.py (5,670 lines)** | CRITICAL | Every model in a single file. | ✅ RESOLVED — split into 9 modules under `models/` |
| **Single App Architecture** | HIGH | All functionality in one Django app. | ⚠️ ACCEPTED — monolith maintained intentionally |
| **No serializers.py** | MEDIUM | API responses are manually constructed dicts. | ⚠️ DEFERRED — no API consumers yet |
| **Wildcard import: `from .models import *`** | MEDIUM | Namespace pollution, unclear dependencies. | ⚠️ PENDING — Phase 5 of clean architecture plan |
| **CustomUser model defined but unused** | MEDIUM | Not set as `AUTH_USER_MODEL`. Dead code. | ⚠️ PENDING |
| **Debug prints in production code** | HIGH | Emoji debug prints in `Visita.save()`. | ✅ RESOLVED |
| **No test coverage** | HIGH | `tests.py` appeared empty. | ✅ PARTIALLY RESOLVED — 5 test files added |
| **results_general.py = 2,150+ lines** | HIGH | Oversized model file. | ⚠️ PENDING — Phase 4 of clean architecture plan |
| **results_anosognosia.py = 1,650+ lines** | HIGH | Oversized model file. | ⚠️ PENDING — Phase 4 of clean architecture plan |
| **4× duplicated exam-mapping dicts** | HIGH | Hardcoded in `exams_dispatch.py` + `visit.py` (×3) | ⚠️ PENDING — Phase 1 of clean architecture plan |
| **No service layer** | MEDIUM | Business logic in views and `save()` overrides | ⚠️ PENDING — Phases 2–3 of clean architecture plan |
...
## SECTION 9: Progress Since v1.0 — What Changed

> Updated: 2026-03-27. Compares original handover (2026-02-21) against current codebase state.

### 9.1 Completed Improvements

| Item | Original State | Current State | Status |
|------|---------------|---------------|--------|
| **Monolithic `models.py` (5,670 lines)** | Single file | Split into 9 modules under `models/` with `__init__.py` re-exports | ✅ DONE |
| **Monolithic `views.py` (10,911 lines)** | Single file | Split into 13 modules under `views/` | ✅ DONE |
| **Monolithic `forms.py`** | 3 forms in one file | Split into `core.py`, `exam_general.py`, `exam_sleep.py`, `exam_anosognosia.py` + `mixins.py` | ✅ DONE |
| **Hardcoded secrets in `settings.py`** | DB password, email, SECRET_KEY in source | All via `python-decouple` `config()` — no hardcoded secrets | ✅ DONE |
| **No HTTPS enforcement** | Missing SSL/HSTS settings | `SECURE_SSL_REDIRECT`, `SECURE_HSTS_SECONDS=31536000`, `SESSION_COOKIE_SECURE`, `CSRF_COOKIE_SECURE` — environment-aware | ✅ DONE |
| **`@csrf_exempt` on `agendar_cita_ajax`** | CSRF bypassed | Removed — CSRF enforced on all endpoints | ✅ DONE |
| **No RBAC** | Binary superuser/authenticated | `role_required()` decorator + `RoleRequiredMixin` with 3 roles (Administrador, Investigador, Evaluador) | ✅ DONE |
| **No signed tokens** | Public endpoints used raw `paciente_id` | `tokens.py` with HMAC-signed time-limited tokens via `django.core.signing` | ✅ DONE |
| **No test suite** | Empty `tests.py` | 5 test files: audit trail, consent signatures, decorators, clarification notes, tokens | ✅ DONE |
| **No structured logging** | Default Django, no config | `LOGGING` configured with security event handler, file rotation | ✅ DONE |
| **No Django Forms on exams** | Raw `request.POST.get()` for all exams | `ExamSaveMixin` + ModelForms for standardized save workflow | ✅ DONE |
| **No `select_related`/`prefetch_related`** | Only 1 usage found | ~18 uses across views (~70% coverage) | ✅ PARTIAL |
| **Rate-limiting** | No protection on public endpoints | Custom `RateLimitMiddleware` on 7 public paths | ✅ DONE |
| **Dead code (`models_old.py`, `forms_old.py`)** | Both existed | Removed | ✅ DONE |
| **`views_old.py`** | 3,700+ lines | **Still exists** | ⚠️ PENDING |

### 9.2 Current Folder Structure (March 2026)
...
├── apps.py
├── config.py
├── decorators.py           # ← NEW: @role_required, ROLES tuple
├── middleware.py            # ← NEW: RateLimitMiddleware
├── posthog_service.py
├── tokens.py               # ← NEW: HMAC signed tokens
├── urls.py
├── views_old.py            # ⚠️ DEAD CODE — still present
├── forms/                  # ← RESTRUCTURED from flat forms.py
│   ├── __init__.py
│   ├── core.py
│   ├── exam_anosognosia.py
│   ├── exam_general.py
│   ├── exam_sleep.py
│   └── mixins.py           # ← NEW: ExamSaveMixin
├── models/                 # ← RESTRUCTURED from monolithic models.py
│   ├── __init__.py
│   ├── analytics.py
│   ├── consent_signature.py
│   ├── patient.py
│   ├── project.py
│   ├── results_anosognosia.py   # 1,650+ lines ⚠️
│   ├── results_general.py       # 2,150+ lines ⚠️ CRITICAL
│   ├── results_sleep.py
│   ├── scheduling.py
│   └── visit.py
├── views/                  # ← RESTRUCTURED from monolithic views.py
│   ├── __init__.py
│   ├── analytics.py
│   ├── auth.py
│   ├── exams_anosognosia.py
│   ├── exams_dispatch.py        # Hardcoded exam_config dict
│   ├── exams_general.py
│   ├── exams_public.py
│   ├── exams_sleep.py
│   ├── patients.py
...
### 9.3 Remaining Technical Debt (Prioritized)

| # | Issue | Severity | Files Affected |
|---|-------|----------|----------------|
| 1 | **40+ hardcoded exam ID/name mappings duplicated in 4 dicts** | HIGH | `exams_dispatch.py`, `visit.py` (×3 methods) |
| 2 | **`results_general.py` = 2,150+ lines** | HIGH | Single file — micro-DDD candidate |
| 3 | **`results_anosognosia.py` = 1,650+ lines** | HIGH | Single file — micro-DDD candidate |
| 4 | **Business logic in `save()` overrides** (patient codes, scoring) | MEDIUM | `visit.py`, `results_anosognosia.py`, `results_general.py` |
| 5 | **Legacy public fallback** — raw `paciente_id` still accepted alongside tokens | MEDIUM | `exams_public.py` |
| 6 | **No pagination** on list views | MEDIUM | `patients.py`, `auth.py` |
| 7 | **Blocking PostHog API calls** in analytics views | MEDIUM | `analytics.py` |
| 8 | **`views_old.py` still in codebase** | LOW | Dead code |
| 9 | **Wildcard imports** in `__init__.py` files | LOW | `models/__init__.py`, `views/__init__.py` |
| 10 | **No composite DB indexes** | LOW | Multiple models |

### 9.4 Current Architecture Diagram
...
└────────────────────────┬─────────────────────────────────────┘
                         │
              ┌──────────▼──────────┐
              │   Apache/Nginx      │  (Bitnami reverse proxy + TLS)
              │   HSTS + SSL        │
              └──────────┬──────────┘
                         │
              ┌──────────▼──────────┐
              │   RateLimitMiddleware│  (30 req/60s on 7 public paths)
              │   Django CSRF       │
              │   @role_required    │  (Administrador/Investigador/Evaluador)
              └──────────┬──────────┘
                         │
              ┌──────────▼──────────┐
              │   Gunicorn WSGI     │  (Port 5005, 1 worker)
   ...
   │  (13 files) │ │(5 files)│ │ (~70 files)   │
   └──────┬──────┘ └────┬────┘ └───────────────┘
          │              │
   ┌──────▼──────────────▼──────┐
   │       models/              │  ← NO SERVICE LAYER
   │      (9 files, ~76 models) │
   └──────────────┬─────────────┘
                  │
          ┌───────▼───────┐
...
**Key gap visible:** Views import models directly. No service layer exists between views and models. Business logic resides in views (email, PDF generation, state transitions) and model `save()` overrides (scoring, code generation).

---

## SECTION 10: Gradual Clean Architecture Plan

> Goal: Introduce clean architecture patterns incrementally without breaking the working system.  
> Principle: Each step must be **backward-compatible**, **independently deployable**, and **testable in isolation**.  
> Constraint: No app splitting. No new Django apps. No domain restructuring. Monolith stays.

### 10.1 Current Layer Assessment
...
│  URL Router (urls.py)                                │
│       │                                              │
│  Views (13 files) ─── Business Logic MIXED HERE      │
│       │                    │                         │
│  Forms (5 files)      Models (9 files)               │
│       │                    │                         │
│       └────────────────────┘                         │
│            Direct ORM access                         │
│            save() overrides with logic               │
│            No service/use-case boundary              │
└─────────────────────────────────────────────────────┘
```

```
┌─────────────────────────────────────────────────────┐
│             TARGET STATE (Gradual)                    │
│                                                      │
│  URL Router (urls.py)                                │
│       │                                              │
│  Views (thin) ── only HTTP concern                   │
│       │                                              │
│  Services (pure Python) ── business logic            │
│       │                                              │
│  Forms (validation)   Models (persistence)           │
│       │                    │                         │
│       └────────────────────┘                         │
│            ORM access via services                   │
│            save() overrides: only DB constraints     │
│            Scoring in pure functions                  │
└─────────────────────────────────────────────────────┘
```
...
**Objective:** Remove noise, establish baselines.

| Step | Action | Risk | Effort |
|------|--------|------|--------|
| 0.1 | Delete `views_old.py` | None | Trivial |
| 0.2 | Remove legacy `paciente_id` fallback in `exams_public.py` — enforce token-only | Low | Small |
| 0.3 | Add missing `db_index=True` on frequently filtered fields | None | Small |
| 0.4 | Add composite indexes via `class Meta: indexes` | None | Small |
| 0.5 | Add pagination to `lista_pacientes` and `administrar_usuarios` | Low | Small |

**Deliverable:** Cleaner codebase, no architectural shift. All existing tests pass.

---

#### PHASE 1 — Exam Registry (Single Source of Truth)

**Objective:** Eliminate the 4× duplicated exam-mapping dicts. This is the highest-value architectural change.

**Current problem:**
```
exams_dispatch.py  →  exam_config = {3: {...}, 4: {...}, ...}     # ID → template/model
visit.py           →  get_url_realizar()  = {"moca": "...", ...}  # name → URL
visit.py           →  get_url_ver()       = {"moca": "...", ...}  # name → URL
visit.py           →  get_url_editar()    = {"moca": "...", ...}  # name → URL
visit.py           →  get_resultado_instance() iterates 30+ related_names
```

**Proposed solution:** Create `apps/home/exam_registry.py`:

```python
# exam_registry.py — Single source of truth for all exam type configurations
#
# Each entry maps an Examen.id to its full configuration:
#   - model: the Result model class
#   - template: template path for the form
#   - related_name: reverse relation on VisitaExamen
#   - urls: dict with keys 'realizar', 'ver', 'editar'
#
# Usage:
#   from apps.home.exam_registry import EXAM_REGISTRY, get_exam_config
#   config = get_exam_config(examen_id=4)
#   config = get_exam_config(exam_name="moca")

EXAM_REGISTRY = {
    # id: { "name": ..., "model": ..., "template": ..., "related_name": ..., "urls": {...} }
}

def get_exam_config(examen_id=None, exam_name=None):
    """Lookup exam configuration by ID or name."""
    ...
```

| Step | Action | Risk | Effort |
|------|--------|------|--------|
| 1.1 | Create `exam_registry.py` populated from existing dicts | None — additive | Medium |
| 1.2 | Write tests: verify registry covers all existing exam IDs and names | None | Small |
| 1.3 | Refactor `exams_dispatch.py` to use registry | Low | Medium |
| 1.4 | Refactor `visit.py` URL methods to use registry | Low | Medium |
| 1.5 | Refactor `get_resultado_instance()` to use registry's `related_name` | Low | Medium |
| 1.6 | Remove old hardcoded dicts | None — after tests pass | Small |

**Deliverable:** Adding a new exam type requires ONE entry in the registry instead of 6 file modifications. Regression tests validate exam routing.

---

#### PHASE 2 — Extract Scoring to Pure Functions

**Objective:** Move scoring formulas out of `save()` overrides into testable pure functions.

**Current problem:** Scoring calculations in `CuidadorNPIResult.save()`, `ExamenFisicoResult.save()` (BMI), and potentially others are tightly coupled to the ORM — impossible to test without database writes.

**Proposed solution:** Create `apps/home/services/scoring.py`:

```python
# services/scoring.py — Pure scoring functions, no ORM dependency
#
# Each function takes raw field values and returns a computed score.
# Models call these in save() — but the logic is testable independently.

def calcular_puntaje_npi(frecuencia_values, gravedad_values):
    """Calculate NPI total score from frequency × severity pairs."""
    ...

def calcular_bmi(peso_kg, talla_cm):
...
| Step | Action | Risk | Effort |
|------|--------|------|--------|
| 2.1 | Create `apps/home/services/__init__.py` + `scoring.py` | None — additive | Small |
| 2.2 | **RED**: Write tests for each scoring formula with known clinical inputs/outputs | None | Medium |
| 2.3 | **GREEN**: Extract pure functions from `save()` methods | Low — models call the function | Small |
| 2.4 | **REFACTOR**: `save()` methods become `self.puntaje_total = calcular_puntaje_npi(...)` | None — same behavior | Small |

**Deliverable:** Scoring logic testable without the database. `save()` overrides are thin wrappers. Clinical integrity protected by regression tests.

---

#### PHASE 3 — Extract Business Operations to Services

**Objective:** Move complex view logic (email, PDF, state transitions) into service functions.

**Current problem:** `firmar_visita` in `visits.py` does: validate → update state → generate PDF → send email → update `firmado_por`. This combines HTTP handling, business rules, I/O, and persistence in one function.
...
├── __init__.py
├── scoring.py              # Phase 2
├── visit_signing.py        # firmar_visita business logic
├── patient_codes.py        # ANG-XXX code generation (from Visita.save())
└── exam_lifecycle.py       # ExamSaveMixin logic as pure service
```

| Step | Action | Risk | Effort |
|------|--------|------|--------|
| 3.1 | Extract `firmar_visita` business logic into `visit_signing.py` | Medium — test coverage needed | Medium |
| 3.2 | Extract patient code generation from `Visita.save()` into `patient_codes.py` | Medium — test first | Medium |
| 3.3 | Views become thin: parse request → call service → return response | Low if tests exist | Medium |

**Deliverable:** Views only handle HTTP. Business logic testable without requests. `save()` overrides reduced to persistence-only concerns.

---

#### PHASE 4 — Split Oversized Model Files

**Objective:** Break `results_general.py` (2,150+ lines) and `results_anosognosia.py` (1,650+ lines) into smaller focused modules.
...
├── results_general/
│   ├── __init__.py              # re-exports
│   ├── examen_fisico.py         # ExamenFisicoResult
│   ├── examen_neurologico.py    # ExamenNeurologicoResult (300+ fields)
│   ├── antecedentes.py          # AntecedentesResult + 10 child models
│   ├── medicamentos.py          # MedicamentosResult + Medicamento
│   ├── revision_sistemas.py     # RevisionSistemasResult + DetalleRevisionSistemas
│   ├── cognitivo_anamnesis.py   # CognitivoAnamnesisResult + child models
│   └── analisis_general.py      # AnalisisGeneralResult + diagnostic models
├── results_anosognosia/
│   ├── __init__.py
│   ├── cognitive.py             # MoCA, CDR, PuntajeCDR
│   ├── caregiver.py             # NPI, Zarit, BettyFerrel, AQDCuidador
│   ├── functional.py            # LawtonBrody, RedLatSpanish
│   ├── quality_of_life.py       # EuroQol5D, Yesavage
│   ├── adherencia.py            # AdherenciaTerapeutica
│   ├── consent.py               # ConsentimientoInformado models
│   └── seguimiento.py           # SeguimientoIntervenciones, Anamnesis
```

| Step | Action | Risk | Effort |
|------|--------|------|--------|
| 4.1 | Split `results_general.py` into sub-package | Low — re-exports maintain backward compatibility | Medium |
| 4.2 | Split `results_anosognosia.py` into sub-package | Low | Medium |
| 4.3 | Verify all imports/migrations still work | Low | Small |

**Deliverable:** No file exceeds 500 lines. Each module is a focused clinical domain. No migration changes needed (Django tracks models by `app_label.ModelName`, not file path).

---

#### PHASE 5 — Replace Wildcard Imports with Explicit Imports

**Objective:** Make dependencies traceable.

| Step | Action | Risk | Effort |
|------|--------|------|--------|
| 5.1 | Replace `from ..models import *` in each view file with explicit imports | None | Medium (mechanical) |
| 5.2 | Replace `from .module import *` in `models/__init__.py` with explicit imports | None | Small |
| 5.3 | Replace `from .module import *` in `views/__init__.py` with explicit imports | None | Small |

**Deliverable:** Every file explicitly declares its dependencies. `grep` shows exactly which models a view uses.
...
Phase 0 (Cleanup)
    │
    ├── Phase 1 (Exam Registry)     ← Highest value, no dependency
    │
    ├── Phase 2 (Scoring Services)  ← Independent of Phase 1
    │       │
    │       └── Phase 3 (Business Services) ← Requires Phase 2 pattern
    │
    ├── Phase 4 (Split Model Files) ← Independent, can run in parallel
    │
    └── Phase 5 (Explicit Imports)  ← Run after Phases 1–4 stabilize
```

**Phases 1, 2, and 4 can proceed in parallel** after Phase 0. Phase 3 depends on Phase 2's service pattern being established. Phase 5 is a mechanical cleanup best done after structural changes settle.

### 10.4 What This Plan Does NOT Do

| Excluded | Reason |
|----------|--------|
| Split into multiple Django apps | Migration chain complexity, import path changes, team disruption |
| Introduce Django REST Framework | No API consumers yet — YAGNI |
| Repository pattern / Unit of Work | Over-engineering for a monolith with 1–5 concurrent users |
| Domain events / event sourcing | Requires infrastructure (message broker) not yet justified |
| CQRS / read models | System is not read-heavy enough to justify |
| Abstract base classes for services | Unnecessary indirection — plain functions are sufficient |

### 10.5 Success Criteria

| Phase | Metric |
|-------|--------|
| 0 | No dead code. All list views paginated. Indexes on filtered fields. |
| 1 | Adding a new exam type requires editing ONE file (`exam_registry.py`) + creating model/form/template. |
| 2 | Every scoring formula has at least 3 test cases (known clinical inputs → expected outputs). |
| 3 | No view function exceeds 30 lines of business logic. Views are HTTP-only wrappers. |
| 4 | No model file exceeds 500 lines. |
| 5 | `grep -r "import \*" apps/home/` returns zero results. |

---

*End of Technical Handover Document v2.0*
